...
export function selfImprovePreamble(taskId: string): string {
  ...
  const protectedFilesList = [...PROTECTED_FILES].join(", ");
  return [
    `# TurboClaw Self-Improve Mode`,
    `# The TurboClaw source is mounted at /project. Work there.`,
    ``,
    `FIRST, run these setup commands before doing anything else:`,
    `\`\`\`bash`,
    `cd /project`,
    `git checkout -b ${branch}`,
    `# Install protected files hook`,
    `mkdir -p .git/hooks`,
    `printf '#!/bin/sh\\nfor f in $(git diff --cached --name-only); do case "$f" in ${[...PROTECTED_FILES].join("|")}) echo "BLOCKED: $f is protected" >&2; exit 1;; esac; done\\n' > .git/hooks/pre-commit`,
    `chmod +x .git/hooks/pre-commit`,
    `\`\`\``,
    ``,
    `Protected files (do not modify): ${protectedFilesList}`,
    ``,
